...
const getAllAnimatorsAssigments = asyncHandler(async (req, res) => {
   // console.log('helloooooooo')
        ...
        if (req.query.id) {
            ...
            const animatorsAssigment = await AnimatorsAssigment.findOne({ _id: id }).lean();
...
console.log(schoolName, schoolCity, schoolType)
    ...
    const duplicate = await AnimatorsAssigment.findOne({schoolName }).lean().exec()//because we re receiving only one response from mongoose

    if (duplicate&&duplicate.schoolType== schoolType) {
        return res.status(409).json({ message: `Duplicate animatorsAssigment: ${duplicate.schoolName}, found` })
    }
  
    
    const animatorsAssigmentObject = { schoolName, schoolCity, schoolType}//construct new animatorsAssigment to be stored

    // Create and store new animatorsAssigment 
    const animatorsAssigment = await AnimatorsAssigment.create(animatorsAssigmentObject)

    if (animatorsAssigment) { //if created 
        res.status(201).json({ message: `New animatorsAssigment of subject: ${animatorsAssigment.schoolName}, created` })
    } else {
        res.status(400).json({ message: 'Invalid animatorsAssigment data received' })
    }
})
...
const updateAnimatorsAssigment = asyncHandler(async (req, res) => {
    ...
    res.json({ message: `animatorsAssigment: ${updatedAnimatorsAssigment.schoolName}, updated` })
})
