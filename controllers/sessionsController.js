...
console.log(schoolName, schoolCity, schoolType)
    ...
    if (duplicate&&duplicate.schoolType== schoolType) {
        return res.status(409).json({ message: `Duplicate session: ${duplicate.schoolName}, found` })
    }
    ...
    if (session) { //if created 
        res.status(201).json({ message: `New session of subject: ${session.schoolName}, created` })
    } else {
